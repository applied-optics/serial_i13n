...
int SERIAL_close(void)
{
	if (SERIAL_set == 1) {
		...
		cout <<
		    "SERIAL_MAIN: close called but interface was not open\n";
...
// allocate buffer static so memory allocation is avoided in loop
	...
	if (SERIAL_buffer == NULL) {
		cout << "SERIAL: Unable to allocate buffer size " << buff_size
		    << " bytes exiting \n";
		return SERIAL_NOMEM;
	}
	SERIAL_set = 1;
...
int SERIAL_get(char *buff)
{
	...
	while (read(SERIAL_in, (void *)SERIAL_buffer, SERIAL_buff_size * 2) !=
	       SERIAL_buff_size * 2) ;
	i = 0;
	while ((SERIAL_buffer[i] < 128) && (i < SERIAL_buff_size * 2))
		i++;
