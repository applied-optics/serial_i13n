...
int serial_open(const char *tty, int baud_rate)
{
	...
	if (SERIAL_DEVICE_NO >= SERIAL_MAX_DEVICES) {
		...
		if ((fd = open(device, O_RDWR)) < 0) {
			fprintf(stderr,
				"serial_open: could not open device %s, ",
				device);
			fprintf(stderr, "the error is %d\n", fd);
			if (fd == -1) {
				fprintf(stderr,
					"Have you set the access permissions?\n");
		...
		if (tcsetattr(fd, TCSANOW, &options) < 0) {
			fprintf(stderr,
				"serial_open: Couldn't set the usb serial port attributes.\n");
		...
		strncpy(SERIAL_TTY[SERIAL_DEVICE_NO], short_device,
			SERIAL_MAX_DEV_LEN);
...
int serial_send_raw(int fd, const char *cmd, int len)
{
	...
	if (ret <= 0) {
		fprintf(stderr,
			"SERIAL_set: select on WRITE return value = %d (should be >0).\n",
			ret);
		   ...
		   const char end_of_message_char)
{
	...
	do {
		...
		res =
		    select(fd + 1, &readfs, (fd_set *) 0, (fd_set *) 0,
			   &Timeout);

		if (res < 0) {
			fprintf(stderr,
				"Serious error: select returned %d (< 0), returning %d\n",
...
//                      fprintf(stderr, "%d bytes read this time, %d bytes read so far\n", bytes_read, total_bytes_read);
	...
	} while (buf[i - 1] != terminate_read_char && i < buf_len
		 && hold_up < 10 && bytes_read != 0);
...
//              fprintf(stderr, "Timed out, and have not read any data (0 bytes). Returning with -3\n");
	...
	if (hold_up == 10) {
		fprintf(stderr,
			"Timed out 10 times, although there was still data coming in.\nVery odd! Returning %d\n",
...
int serial_create_lockfile(const char *device_name)
{
	...
	snprintf(serial_lockfile, SERIAL_MAX_DEV_LEN, "%s%s",
		 SERIAL_LOCKFILEDIR, device_name);
	if (access(serial_lockfile, F_OK) == 0) {
		...
		if (count >= 3) {
			fprintf(stderr, "Lockfile %s found.\n",
				serial_lockfile);
			fprintf(stderr,
				"It is being held by process with PID %d, which is still running.\n",
				id);
			fprintf(stderr,
				"Cannot release lock, returning with exit code %d.\n",
	...
	if (access(SERIAL_LOCKFILEDIR, W_OK) != 0) {
		fprintf(stderr,
			"No lock file found (good); however, cannot write to directory %s\n",
			SERIAL_LOCKFILEDIR);
